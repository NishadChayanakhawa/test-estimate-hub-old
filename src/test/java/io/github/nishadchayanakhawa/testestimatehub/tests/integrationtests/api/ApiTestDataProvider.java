...
public class ApiTestDataProvider {
	...
	private static Object[][] toObjectArray(JSONObject json) throws JsonFileHandlingException, JSONException {
		JSONArray jsonArray;
		try {
			jsonArray=json.getJSONArray(ApiTestDataProvider.TEST_DATA_JSON_KEY);
	...
	public Object[][] getTestDataFromJson(ITestNGMethod testNGMethod) throws FlatFileHandlingException, JsonFileHandlingException, JSONException {
		String className=ApiTestDataProvider.getClassName(testNGMethod);
		String methodName=ApiTestDataProvider.getMethodName(testNGMethod);
		
		String jsonTestDataFileFullPath=
				String.format("%s%s/%s.json", ApiTestDataProvider.TEST_DATA_BASE_LOCATION,className,methodName);
		JSONObject testDataJson=null;
		try {
			testDataJson = JsonFileHandling.getJsonFileContent(jsonTestDataFileFullPath);
			return ApiTestDataProvider.toObjectArray(testDataJson);
